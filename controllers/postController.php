...
class postController extends Controller
{
    ...
    public function nuevo()
    {
        ...
        if ($this->getINT('guardar') == 1) {
            ...
            if (!$this->getTexto('titulo')) {
                $this->_view->_error = 'Debe instroducir el titulo del post';
                $this->_view->renderizar('nuevo', 'post');
                exit;
            }
            if (!$this->getTexto('cuerpo')) {
                $this->_view->_error = 'Debe instroducir el cuerpo del post';
                $this->_view->renderizar('nuevo', 'post');
                exit;
            }
            $this->_post->insertarPost(
                $this->getPostParam('titulo'), //$_POST['titulo'],//$this->getTexto('titulo'),
    ...
    public function editar($id)
    {
        ...
        if ($this->getINT('guardar') == 1) {
            ...
            $this->_post->editarPost($this->filtrarInt($id), $this->getPostParam('titulo'), $this->getPostParam('cuerpo'));
            $this->redireccionar('post');
        }
